from toolbox.graph import longuestTrail, compressUnoriented
...
def prepare(src: str) -> (dict, int, int):
    ...
    for j in range(len(data[0])):
        ...
        for i in range(len(data)):
            ...
            if i_orig != -1 and data[i][j] == '#':
                if i - i_orig > 1:
                    addVertex((i_orig, j), (i - 1, j), i - i_orig - 1)  # Check if not length 1

                i_orig = -1

            # Check intersection
            if i_orig != -1 and i - i_orig > 1 and ((j > 0 and data[i][j - 1] != '#') or (j < len(data[0]) - 1 and data[i][j + 1] != '#')):
                addVertex((i_orig, j), (i, j), i - i_orig)
