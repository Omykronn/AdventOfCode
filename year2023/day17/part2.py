            ...
            for new_position in [turnLeft(position), turnRight(position)]:
                ...
                for nb_step in range(10):
                    ...
                    if isLegit(new_position, len(data), len(data[0])):
                        ...
                        # First condition might be simplified, but it is easier to read like this
                        # and we save a new minimum of heat loss only if it is reached according the rules
                        # otherwise it will create minima that one is unable to reach with the rules
                        if nb_step + 1 >= 4 and sum_heat_loss < cost[new_position[0]][new_position[1]][new_position[2]]:
